...
* **검증 결과 (Docker 2vCPU / 4GB 모사 환경):**
  ...
  * DB I/O 대기 소멸로 애플리케이션 CPU 점유율 219%에서 58%로 감소
    ...
    | 측정 지표 (500 TPS 기준) | AS-IS (Sync + No Cache) | TO-BE (@Async + Ehcache) | 개선 효과 |
    | :--- | :--- | :--- | :--- |
    | **평균 처리량 (TPS)** | 측정 불가 (시스템 마비) | **437 TPS** | **목표치(500 TPS) 근접 수용** |
    | **P(95) Latency** | 1.53s | **8.92ms** | **99% 단축** |
    | **Error Rate** | 99.99% | **0.00%** | **시스템 가용성 100% 확보** |
    | **Dropped Iterations** | 11,147건 | **29건** | **대기열 포화 99.7% 해소** |
    | **App CPU 사용량** | 219.35% (Overload) | **58.12% (Stable)** | **부하 73% 감소** |
